...
//        }
        ...
        $filterPictures = $this->getFilteredPictures($bucket, [
            'id' => $param['id'],
    ...
    private function getAllBucketPictures($bucket, $prefix, &$clearCache = false)
    {
        $cache = \Bitrix\Main\Application::getInstance()->getManagedCache();

        if($clearCache){
            $cache->clean($bucket . $prefix);
        }

        if ($cache->read($this->cacheTime, $bucket . $prefix)) {

            $content = $cache->get($bucket . $prefix);
            return $content;
        }
        else {
            ...
            if( count($content) ) {
                ...
                $content = $pics;
                $cache->set($bucket . $prefix, $content);
                return $content;
            }
        }
    ...
    private function getFilteredPictures($bucket, array $filter, $clearCache = false)
    {

        $pictures = $this->getAllBucketPictures($bucket, $filter['id'], $clearCache);
        ...
        if( !$clearCache && !count($pictures) ){
            $clearCache = true;
            return $this->getFilteredPictures($bucket, $filter, $clearCache);
        }
