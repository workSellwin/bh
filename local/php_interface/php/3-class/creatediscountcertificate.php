...
class CreateDiscountСertificate
{
    ...
    public function GetCode($sum)
    {
        ...
        foreach ($arId as $id) {
            if ($id) {
                ...
                $code[] = $newCode;
                $this->CouponAdd( self::CouponAddFields($id, $newCode, $sum) );
            }
        }
        return implode(' ,', $code);
    }
     ...
     * @return array
     */
    protected function CouponAddFields($id, $code, $description = "")
    {
        return [
            "DISCOUNT_ID" => $id,
            "COUPON" => $code,
    ...
    protected function DiscountAddFields($s, $sum)
    {
        ...
        return array(
            ...
            'SORT' => '1',
            'PRIORITY' => '7788',
            'LAST_DISCOUNT' => $USER->IsAdmin() ? 'Y' : 'N',
            'LAST_LEVEL_DISCOUNT' => $USER->IsAdmin() ? 'Y' : 'N',
            'XML_ID' => '',
            'CONDITIONS' => array(
                'CLASS_ID' => 'CondGroup',
                'DATA' => array(
