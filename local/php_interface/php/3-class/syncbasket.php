    ...
    public static function HandlerOnBasketUpdate($id, $arFields)
    {
        ...
        if ($arFields['LID'] == $ob->GetThisSite()) {
            if ($otherId = $ob->GetIdOther($id)) {
                ...
                if( $_GET['print'] == 'yes' ) {
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/local/sb.txt', $ob->GetThisSite() . "\n", FILE_APPEND);
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/local/sb.txt', $otherId . "\n", FILE_APPEND);
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/local/sb.txt', print_r($_SERVER['DOCUMENT_ROOT'], true) . "\n", FILE_APPEND);
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/local/sb.txt', print_r($_SERVER['SERVER_NAME'], true) . "\n", FILE_APPEND);
                    file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/local/dbtr.txt', print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), true) . "\n", FILE_APPEND);
                }
