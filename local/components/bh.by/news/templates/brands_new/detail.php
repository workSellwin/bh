...
$arFilter = Array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "CODE" => $arResult['VARIABLES']['ELEMENT_CODE'], "ACTIVE" => "Y");
$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
while ($ob = $res->GetNext()) {
    $arBrendElem['BREND'] = $ob;
    if($ob['PROPERTY_SITE_VALUE']){
        $arBrendElem['SITE_ID'][$ob['PROPERTY_SITE_VALUE']] = $ob['PROPERTY_SITE_VALUE'];
    }
    if($ob['PROPERTY_SERIES_ID']){
        $arBrendElem['SER_ID'][$ob['PROPERTY_SERIES_ID']] = $ob['PROPERTY_SERIES_NAME'];
    }
    if($ob['PROPERTY_SERIES_CODE']){
        $arBrendElem['SERII_CODE'][$ob['PROPERTY_SERIES_ID']] = $ob['PROPERTY_SERIES_CODE'];
    }
}
...
if(SITE_ID == 's2'){
    if(in_array('s1', $arBrendElem['SITE_ID']) && !in_array('s2', $arBrendElem['SITE_ID'])){
        LocalRedirect("https://bh.by".$arBrendElem['BREND']['DETAIL_PAGE_URL'], false, '301 Moved Permanently');
    }
}
            ...
            <div id="MY_AJAX_FILTER">
                ...
                <?//if (!isset($_REQUEST['no_serii']) && $_REQUEST['no_serii'] != 'Y'):?>
                    ...
                    if(empty($arBrendElem['SER_ID'])){
                        ...
                        LocalRedirect($APPLICATION->GetCurPage().'?no_serii=Y', false, '301 Moved Permanently');

                    } ?>

                    <div class="btn btn_border" data-use="show-more-4" onclick="location='<?=$arBrendElem['BREND']['DETAIL_PAGE_URL']?>?no_serii=Y'">Показать всё</div>
