...
$this->addExternalCss('/local/templates/.default/css/bootstrap.css');
...
else
{
	...
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
        ...
        <input type="hidden" name="ELEM_GIFTS_ID" value=""/>
		<div id="bx-soa-order" class="bx-<?=$arParams['TEMPLATE_THEME']?> cl" style="opacity: 0">
			<!--	MAIN BLOCK	-->
			<div class="col-sm-9 bx-soa">
				...
				<? if ($arParams['BASKET_POSITION'] === 'after'): ?>
					<!--	BASKET ITEMS BLOCK	-->
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active hide">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-count"></span><?=$arParams['MESS_BASKET_BLOCK_NAME']?>
							</h2>
                ...
                <div style="color: red">Проверяйте заказ при курьере! Только на данном этапе можно отказаться от товара.<br>Отказ и возврат оплаченного товара невозможен.</div>
