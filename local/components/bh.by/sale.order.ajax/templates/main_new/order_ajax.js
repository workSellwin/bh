...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            this.orderPaySystemConfirm = true;
        ...
        refreshOrder: function (result) {
            ...
            //Yauheni_4---------------------------------------------------------------------------------------------------------
            var data = result;
            if(data['order']['ORDER_PROP']['properties']){
                for (var indexProp in data['order']['ORDER_PROP']['properties']) {
                    var prop = data['order']['ORDER_PROP']['properties'][indexProp];
                    if (prop['ID'] == 46) {
                        getTerminalCity();
                    }
                }
        ...
        fade: function (node, nextSection) {
            ...
            $('#soa-property-3').mask("+375999999999");
        ...
        editOrder: function () {
            ...
            for (var i in this.result.DELIVERY) {
                if (this.result.DELIVERY[i].CHECKED == 'Y') {
                    var sumTotalNds = parseFloat(this.result.DELIVERY[i].PRICE);
                }
            }

            function addNdsOrder(price, nds) {
                if (nds) {
                    var returnPrice;
            ...
            var sumTotalBasketNds = 0;
            var currency;
            for (var i in this.result.GRID.ROWS) {
                var price = parseFloat(this.result.GRID.ROWS[i].data.PRICE) * parseInt(this.result.GRID.ROWS[i].data.QUANTITY);
                var priceNds = addNdsOrder(price, $('#sum_nds_' + i).attr('data-nds'));
                currency = this.result.GRID.ROWS[i].data.CURRENCY;
                sumTotalBasketNds = sumTotalBasketNds + priceNds;
            }
            //todo nds
            /*var sumTotalNdsFormat = BX.Currency.currencyFormat(sumTotalBasketNds, currency, true);
			$('.bx-soa-cart-total').append('<div class="bx-soa-cart-total-line bx-soa-cart-total-line-total gmi-nds"><span class="bx-soa-cart-t">Итого с НДС:</span><span class="bx-soa-cart-d bx-soa-changeCostSign" style="font-size: 28px;">'+sumTotalNdsFormat+'</span></div>');*/
        },
        ...
        editPaySystemInfo: function (paySystemNode) {
            ...
            if (currentPaySystem) {
                ...
                extPs = BX.create('DIV', {children: [subTitle, title, price]});
            }
        ...
        getInnerPaySystem: function () {
            ...
            // LUIWADJOGS
            if (this.result.PAY_CURRENT_ACCOUNT == 'Y') {
                this.result.CURRENT_BUDGET_FORMATED = parseFloat(this.result.CURRENT_BUDGET_FORMATED.replace(/\s/g, '')) - parseFloat(this.result.TOTAL.PAYED_FROM_ACCOUNT_FORMATED.replace(/\s/g, ''));
                this.result.CURRENT_BUDGET_FORMATED = this.result.CURRENT_BUDGET_FORMATED + ' руб.';
            }
        ...
        selectPaySystem: function (event) {
            ...
            function ConfirmPaySystem(mes, self) {
                        ...
                        new BX.PopupWindowButton({
                            ...
                            events: {
                                click: function () {
                                    ...
                                    self.orderPaySystemConfirm = false;
                                    $('div.bx-soa-pp-company-image:last').trigger('click');
                                }
                            }
                        }),
                        new BX.PopupWindowButton({
            ...
            if (event.path) {
                if (event.path.length) {
                    if (this.orderPaySystemConfirm && event.path.length == 19) {
                        if (!ConfirmPaySystem('Сумма будет списана с Вашего Баланса', this)) {
                            return false;
                        }
                    } else if (event.path.length == 17) {
                        this.orderPaySystemConfirm = true;
                    }
                }
            }
