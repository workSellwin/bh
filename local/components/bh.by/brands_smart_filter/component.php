...
if($this->StartResultCache(false, 'v9'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		...
		else
		{
			...
			//PR($arParams['BRANDS_ELEM_FILTER']);
			if ('Y' == $this->arParams['HIDE_NOT_AVAILABLE'])
				$arElementFilter['CATALOG_AVAILABLE'] = 'Y';

			if(!empty($arParams['FILTER_PROP']) && isset($arParams['FILTER_PROP'])){
                $arElementFilter = array_merge($arElementFilter, $arParams['FILTER_PROP']);
            }
