 ...
 * @var array $result
 */
if(!empty($result['PRICE_WITHOUT_DISCOUNT'])) {
    $priceWithoutDiscount = floatval( preg_replace("/[^,.0-9]/", '', $result['PRICE_WITHOUT_DISCOUNT']));
}
else{
    $priceWithoutDiscount = 0;
}
foreach($result['GRID']['ROWS'] as $key => &$basketItem){
    if(in_array($basketItem['PRODUCT_ID'], ['43463', '43461', '43462'])){
        if(floatval($basketItem['PRICE']) > 0){
            unset($result['GRID']['ROWS'][$key]);
            $basket = Sale\Basket::loadItemsForFUser(Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite());
            $basket->getItemById($basketItem['ID'])->delete();
            $basket->save();
            header('Location: /personal/cart/');
            exit;
        }
        $priceWithoutDiscount -= $basketItem['BASE_PRICE'];
        //PR($basketItem);
        $basketItem['BASE_PRICE'] = 0;
    }
}
$result['PRICE_WITHOUT_DISCOUNT'] = $priceWithoutDiscount . ' руб.';
$result['DISCOUNT_PRICE_FORMATED'] = ($priceWithoutDiscount - $result['allSum']) . ' руб.';
$result['DISCOUNT_PRICE_ALL_FORMATED'] = ($priceWithoutDiscount - $result['allSum']) . ' руб.';
