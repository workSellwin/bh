...
function updateBasketTable(basketItemId, res)
{
		...
		if (createNewItem)
		{
			...
			for (i = 0; i < arColumns.length; i++)
			{
				switch (arColumns[i])
				{
					...
					case 'QUANTITY':
						...
						oCellQuantityHTML += '<div class="quantity">';
						oCellQuantityHTML += '<a href="javascript:void(0);" class="plus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'up\', ' + bUseFloatQuantity + ');"></a>';
						...
						oCellQuantityHTML += '<a href="javascript:void(0);" class="minus" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');"></a>';
			...
			if (bShowDeleteColumn || bShowDelayColumn)
			{
				...
				if (bShowDeleteColumn)
					oCellControl.innerHTML = '<a class="del-cart" href="' + basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) + '">' + basketJSParams['SALE_DELETE'] + '</a><br />';
	...
	if (!!res.BASKET_DATA)
	{
		...
		if(res['BASKET_DATA']['allSum'] <= 30){
			var sum_dostavki = (30 - res['BASKET_DATA']['allSum']);
			sum_dostavki = +sum_dostavki.toFixed(2);

			$(".min-sum-order span.sum").text(sum_dostavki+' руб.');

			$(".min-sum-order").css("display","block");
		}else{
			$(".min-sum-order").css("display","none");
		}
		...
		var sumNdsAll = 0;
		for (var i in res.BASKET_DATA.BASKET_ITEM_RENDER_DATA) {
			var nds = $('#sum_nds_'+res.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i].ID).attr('data-nds');
			var sumNds = addNds(parseFloat(res.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i].SUM_PRICE),nds);
			$('#sum_nds_'+res.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i].ID).html(BX.Currency.currencyFormat(sumNds, res.BASKET_DATA.CURRENCY, true));
			sumNdsAll = parseFloat(sumNdsAll) + parseFloat(sumNds);
		}

		$("#allVATSum_FORMATED_NDS").attr('data-sum-value',sumNdsAll);
		$("#allVATSum_FORMATED_NDS").html(BX.Currency.currencyFormat(sumNdsAll, res.BASKET_DATA.CURRENCY, true));
...
function couponCreate(couponBlock, oneCoupon)
{
	...
	//Yauheni_4 ----------------
	var text_coupon = '';
	if(couponClass == 'disabled' && (oneCoupon.COUPON == 'LOVEBH'|| oneCoupon.COUPON == 'birthday')){
		text_coupon = 'Внимание! Данный промокод дает скидку только на товары сайта bh.by (без учета акционных товаров) и не применяется для данного набора корзины'
	}
	//--------------------------
