...
if (empty($arResult["ITEMS"]))
    echo "<div class='container'>Сожалеем, но ничего не найдено.</div>";
...
?>
<div class="product__list" data-entity="<?= $containerName ?>">
    <?
...
<? if ($arResult['POPULAR_LINK']): ?>
    <? $this->SetViewTarget('popular-link'); ?>
    <div class="product__popular-lnk">
        <h2>Популярное в категории</h2>
        <ul>
            <? $curPage = $APPLICATION->GetCurPage(false); ?>
            <? foreach ($arResult['POPULAR_LINK']['PROPERTIES']['LINK']['VALUE'] as $keyLnk => $itemLnk): ?>
                <li>
                    <? if (strpos($curPage, $arResult['POPULAR_LINK']['PROPERTIES']['LINK']['DESCRIPTION'][$keyLnk]) !== 0): ?>
                        <a href="<?= $arResult['POPULAR_LINK']['PROPERTIES']['LINK']['DESCRIPTION'][$keyLnk] ?>"><?= $itemLnk ?></a>
                    <? else: ?>
                        <span><?= $itemLnk ?></span>
                    <? endif ?>
                </li>
