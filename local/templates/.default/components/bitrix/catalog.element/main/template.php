...
?>
    ...
    <div class="container">
        <div class="prod cl" id="<?= $itemIds['ID'] ?>" itemscope itemtype="http://schema.org/Product">
            ...
            <meta itemprop="brand" content="<?= $arResult['PROPERTIES']['BRANDS']['VALUE'] ?>"/>
            ...
            if ($haveOffers) {
                foreach ($arResult['JS_OFFERS'] as $offer) {
                    ...
                    <span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                        ...
                        <meta itemprop="url" content="<?= $APPLICATION->GetCurPage() ?>"/>
					...
					<? if ($arParams['SHOW_DISCOUNT_PERCENT'] === 'Y') {
                        if ($haveOffers) {
                            ...
                            <span class="prod-status__item prod-status__item-describe"
                                  ...
                                  style="display: none; background-color: #f42561;">
							</span>
                            <?
                        } else {
                            if ($price['DISCOUNT'] > 0) {
                                ...
                                <span id="<?= $itemIds['DISCOUNT_PERCENT_ID'] ?>"
                                      title="<?= -$price['PERCENT'] ?>%">
									<span style="background-color: #f42561;"
                                          class="prod-status__item prod-status__item-describe"><?= -$price['PERCENT'] ?>%</span>
								</span>
                                <?
                            }
                        }
                    } ?>
                    <? ShowLabelFavourably('element', $arResult['PROPERTIES']) ?>
                    <? //ShowLabelFavourably('element_img', $arResult['PROPERTIES']) ?>
				</span>
                                ...
                                if (!empty($actualItem['MORE_PHOTO'])) {
                                    $arWaterMark = Array(
                                        array(
                                            ...
                                            "size" => "100%",
                                            'alpha_level' => 100,
                                            "file" => $_SERVER["DOCUMENT_ROOT"] . "/upload/i/water.png",
                                            "fill" => "exact",
                                        )
                                    ...
                                    foreach ($actualItem['MORE_PHOTO'] as $key => $photo) {
                                        ...
                                        $img_resized = CFile::ResizeImageGet($photo['ID'], array('width' => 518, 'height' => 518), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arWaterMark);
                ...
                <? if (!empty($arResult['DISPLAY_PROPERTIES']) || $arResult['SHOW_OFFERS_PROPS']): ?>
                    ...
                    if (!empty($arResult['DISPLAY_PROPERTIES'])) {
                        ...
                        <div class="prod__short-info">
                            ...
                            <a class="j-description" href="#description2">Перейти к описанию</a>
                            <? unset($property);
                            ?>
                        </div>
                        <?
                        ...
                        foreach ($arResult['SKU_PROPS'] as $skuProperty) {
                            ...
                            <div class="product-item-detail-info-container" data-entity="sku-line-block">
                                <div class="product-item-detail-info-container-title">Выберите
                                    номинал<?//=htmlspecialcharsEx($skuProperty['NAME'])
                                    ?>:
                                </div>
                                                        ...
                                                        <li class="product-item-scu-item-color-container"
                                                            ...
                                                            <div class="product-item-scu-item-color-block">
                                                                <div class="product-item-scu-item-color"
                                                                     title="<?= $value['NAME'] ?>">
                                                                    <img src="<?= ($src) ? $src : $value["PICT"]['SRC'] ?>"
                                                                         alt="<?= $value['NAME'] ?>">
                                                                </div>
                                                            </div>
                                                            <?
                ...
                <div class="prod__price">
                    ...
                    <div class="prod__price-current" id="<?= $itemIds['PRICE_ID'] ?>">
                        ...
                        if ($price['PRICE_TYPE_ID'] == 2) {
                            ...
                            $priceDeliveri = (float)$jsParams['NDS_PRICE'];
                            echo CurrencyFormat($jsParams['NDS_PRICE'], $price['CURRENCY']);
                        } else {
                            $priceDeliveri = (float)$price['PRINT_RATIO_PRICE'];
...
<?if( $USER->IsAdmin() || true ):?>
    <style>
        ...
        #found-cheaper .form-field, #found-cheaper .popup-check-sale__submit-wrap{
            text-align: center;
            padding-bottom: 15px;
        }
        #found-cheaper .form-field input{
            text-align: center;
            padding-bottom: 15px;
            line-height: 1.6;
        }
        #found-cheaper-ok form{
            text-align: center;
        }
        .validation-error{
            border-style: none;
            box-shadow: inset 0px 0px 3px 2px firebrick;
        }
    </style>
    <a class="found-cheaper" data-fancybox data-src="#found-cheaper" href="javascript:;">Нашли&nbsp;дешевле?</a>

    <div id="found-cheaper" class="popup popup-check-sale preloader-wrap" style="display: none;">
        <a href="#" onclick="$.fancybox.close();return false;" class="popup__fancybox-close"></a>
        <h2 class="popup__title">Заполните поля ниже, и мы вышлем Вам промокод со скидкой на указанный номер телефона.</h2>

        <div class="double-separator"></div>

        <div class="popup-check-sale__form-wrap">
            <div class="ajax-message"></div>
            <form action="" method="post" class="loyalty__form-check">
                <div class="two-columns-grid">
                    <div class="two-columns-grid__item">
                        <div class="form-field">
                            <input name="phone" class="form-field__input form-control bx-soa-customer-input bx-ios-fix"
                                   data-validation-type="phone" value="" placeholder="Телефон">
                        </div>
                    </div>

                    <div class="two-columns-grid__item">
                        <div class="form-field">
                            <input name="href" value="" class="form-field__input" type="text"
                                   data-validation-type="text" data-validation-length="5" placeholder="Ссылка на сайт, где цена дешевле">
                        </div>
                    </div>
                </div>

                <div class="popup-check-sale__submit-wrap">
                    <input type="submit" value="Отправить" class="btn btn_border var-2-option ">
                </div>
            </form>
        </div>
    ...
    <div id="found-cheaper-ok" class="popup popup-check-sale preloader-wrap" style="display: none;">
        <a href="#" onclick="$.fancybox.close();return false;" class="popup__fancybox-close"></a>
        <h2 class="popup__title">Заявка принята. Наш менеджер отправит Вам промокод на скидку в ближайшее время.</h2>
        ...
        <div class="popup-check-sale__form-wrap">
            ...
            <form action="" method="post" class="loyalty__form-check">
                <div class="popup-check-sale__submit-wrap">
                    <input type="button" value="Ok" class="btn btn_border var-2-option" onclick="$.fancybox.close();return false;">
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        var btnFoundCheaper = document.querySelector('.popup-check-sale__submit-wrap input');
        btnFoundCheaper.addEventListener('click', function(event){
            event.preventDefault();

            if( !window.cheapFormValidator.check() ){
                return false;
            }

            var phone = document.querySelector('#found-cheaper input[name="phone"]').value;
            var href = document.querySelector('#found-cheaper input[name="href"]').value;
            // var uri = 'phone=' + phone + '&href=' + href;
            // uri = encodeURI(uri);

            var xhr = new XMLHttpRequest();
            xhr.open('post', '/ajax/cheap.php');
            ...
            var sendData = JSON.stringify({
                phone: phone,
                href: href,
                vendor_code: "<?= $arResult['PROPERTIES']['ARTNUMBER']['VALUE'] ?>"
            });
        ...
        window.cheapFormValidator = new FormValidator('#found-cheaper form');
        window.cheapFormValidator.init();
    </script>
<?endif;?>
                </div>
                <div class="js-timer-wrp"></div>
                    ...
                    <div class="prod__btn-container product_id_<?= $arResult["ID"] ?> product-item-button-container"
                        ...
                        <? if ($flagSalon): ?>
                            ...
                            <a class=" btn btn-default product-item-detail-buy-button"
                               ...
                               href="javascript:void(0);">
                                <span><?= Loc::getMessage('BY_BTN_TEXT');?></span>
                            </a>
                        <? endif; ?>
                        <a class="btn btn-default product-item-detail-buy-button"
                        ...
                        <div style="" id="one-click">
                            ...
                            <script>
                                ...
                                // var oneClickForm = new FormValidator('form.one-click-form');
                                // oneClickForm.init();
                                //
                                // document.querySelector('.one-click-form').addEventListener('submit', function (event) {
                                //     if(!oneClickForm.check()){
                                //         event.preventDefault();
                                //     }
                                // }, true)
                            </script>
                        ...
                        if ($USER->IsAuthorized()) { ?>
                            <?
                            $view = PriceMonitor::getInstance()->view(['UF_USER_ID' => $USER->GetID(), 'UF_PRODUCT_ID' => $arResult['ID']]);
                            ?>
                            <div id="price-monitor">
                                <span data-id="<?= $arResult['ID'] ?>"
                                      class="btn btn_border var-1-option <? echo $view ? 'hide' : '' ?> ">Следить за ценой</span>
                                <span data-id="<?= $arResult['ID'] ?>"
                                      class="btn btn_border var-2-option <? echo $view ? '' : 'hide' ?>">В листе ожидания</span>
                            </div>
                        <? } ?>
                        <div class="fs-12">
                            <?
                            $APPLICATION->IncludeComponent(
                                "bh:delivery.text.detail",
                                "",
                                array('PRICE' => $priceDeliveri)
                            );
                            ?>
                        </div>
        ...
        <?// //Yauheni_4 -----------------------------------------------------------------?>
            <?
            global $USER;
            if (isset($_REQUEST['product_buy_js']) && $_REQUEST['product_buy_js'] == 'Y'):
                        ...
                        <div class="container">
                            <div class="prod-other">
                                ...
                                if(isset($_REQUEST['prod_name']) && !empty($_REQUEST['prod_name'])){
                                    ...
                                    $arFilter = Array("IBLOCK_ID"=>$arResult['IBLOCK_ID'], "ACTIVE"=>"Y", "NAME"=>isset($_REQUEST['prod_name']) ? $_REQUEST['prod_name']: '' );
                                ...
                                global $arrFilter_product_buy;
                                $arrFilter_product_buy = array("ID" => [$arResult["ID"], $prod_id]);
                                ...
                                $APPLICATION->IncludeComponent(
                                    ...
                                    array(
                                        "TITLE" => $title,
                                        "DIC_ID" => isset($_REQUEST['not_dic_id']) && $_REQUEST['not_dic_id'] == 'N'  ? $arResult["ID"] : $prod_id ,
                                        "ARRAY_PRODUCT_ID" => array($arResult["ID"], $prod_id),
                                        ...
                                        "ENLARGE_PROP" => "-",
                                        "FILTER_NAME" => "arrFilter_product_buy",
                    ...
                    <? if (!empty($arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE'])): ?>
                        <div class="video-block-section">
                            <div class="video-block">
                                ...
                                $embedUrl = preg_replace($youtubeRegexp, '/embed/$2', $arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE']);
                                $pi = pathinfo($arResult['PROPERTIES']['VIDEO_YOUTUBE']['VALUE']);
                                $ext = $pi['extension'];
                                if ($ext == 'mp4') {
                                    ?>
...
?>
    ...
    <script type="text/javascript">
                    ...
                    "products": [
                        {
                            ...
                            "name": "<?=$arResult['NAME']?>",
                            "price": +price[0],
                            "brand": "<?=$arResult['PROPERTIES']['BRANDS']['VALUE']?>",
                            "category": "<?= $arResult['SECTION']['NAME'] ?>",
                            "variant": "Артикул: <?=$arResult['PROPERTIES']['ARTNUMBER']['VALUE']?>, Объём: <?=$arResult['PROPERTIES']['SCOPE']['VALUE']?>, Производитель: <?=$arResult['PROPERTIES']['MANUFACTURER']['VALUE']?>"
                        }
                    ]
                }
            }
        ...
        var url_img = $('.prod__big-img__item .img').attr('src');
        ...
        carrotquest.identify([
            {op: "union", key: "$viewed_products", value: "<?=$arResult['NAME']?>"}
        ]);
        //---------------------------------------------
