...
if (!empty($arResult['MORE_PHOTO'])) {
    foreach ($arResult['MORE_PHOTO'] as $key => &$photo) {
        $img_resized = CFile::ResizeImageGet(CFile::GetFileArray($photo['ID']), array('width' => 518, 'height' => 518), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        $photo["new_src"] = $img_resized["src"];
    }
}
...
$arResult['IS_BUY_USER'] = $ob->isBuy($arResult['ID']);

if(!$arResult['IS_BUY_USER']){
    $arResult['CATALOG_QUANTITY']=0;
    $arResult['CAN_BUY']='';
    $arResult['ITEM_PRICES']=[];
}
//PR($arResult['IS_BUY_USER'],true);