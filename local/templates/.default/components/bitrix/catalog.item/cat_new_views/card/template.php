...
<div class="prod-item  <?= $arParams['DATA_LIST'] == 'S' ? 'new_views_item' : '' ?>  ">
    ...
    <div class="child child-30 child-center">
        ...
        <a class="product-item-image-wrapper prod-img" href="<?= $item['DETAIL_PAGE_URL'] ?>" title="<?= $imgTitle ?>"
           data-entity="image-wrapper">
            ...
            <span class="my_views_item_span" id="<?= $itemIds['PICT'] ?>"
                  style="display: <?= ($showSlider ? 'none' : 'block') ?>;">
                <?
                $file = CFile::ResizeImageGet($item['PREVIEW_PICTURE']['ID'], array('width' => 283, 'height' => 346), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                ?>
                <?if(/*$_GET['ll'] == 'Y' &&*/ intval($arParams['INDEX']) > 6):?>
                    <img data-index="<?=$arParams['INDEX'];?>" src="/local/templates/.default/images/zaglushka.png" data-defer-src="<?=$file['src']?>" alt="<?= $productTitle ?>"/>
                <?else:?>
            ...
            foreach ($item['LABEL_ARRAY_VALUE'] as $code => $value) {
                ...
                if ($code == 'NEWPRODUCT'):?>
                    <span<?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' class="hide"' : '') ?>>
					    <span title="<?= $value ?>"
                              class="prod-status__item <?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? 'hide' : '') ?>"
                              style="background-color: #5bccd8;"><?= $value ?></span>
				    </span>
                <? elseif ($code == 'SALELEADER'):?>
                    <span<?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' class="hide"' : '') ?>>
					    <span title="<?= $value ?>"
                              class="prod-status__item <?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? 'hide' : '') ?>"
                              style="background-color: #b0ea24;"><?= $value ?></span>
				    </span>
                <? else:?>
                    <span<?= (!isset($item['LABEL_PROP_MOBILE'][$code]) ? ' class="hide"' : '') ?>>
					    <span title="<?= $value ?>"
    ...
    <div class="child child-50">
        <a class="prod-name" title="<?= $productTitle ?>"
