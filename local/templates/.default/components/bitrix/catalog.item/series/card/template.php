...
<div class="related__item">
    <a id="favorites_<?= $item["ID"] ?>" class="favorites-lnk js-favorites-lnk" data-ajax="/ajax/add_favorites.php"
       data-id="<?= $item["ID"] ?>" data-status="Y" href="#"></a>
    ...
    <a class="product-item-image-wrapper related__img" href="<?= $item['DETAIL_PAGE_URL'] ?>" title="<?= $imgTitle ?>"
       data-entity="image-wrapper">
        ...
        <span id="<?= $itemIds['PICT'] ?>" style="display: <?= ($showSlider ? 'none' : '') ?>;">
    ...
    <a class="related__name" title="<?= $productTitle ?>"
       href="<?= $item['DETAIL_PAGE_URL'] ?>"><?= $productTitle ?></a>
    <div class="related__hiden">
        <div class="prod-price" data-entity="price-block">
            <?
            if ($arParams['SHOW_OLD_PRICE'] === 'Y') {
                ?>
                <div class="prod-price__old" id="<?= $itemIds['PRICE_OLD'] ?>"
            ...
            ?>
            <div class="prod-price__new" id="<?= $itemIds['PRICE'] ?>">
                <?
                if (!empty($price)) {
                    //TODO добавляем НДС к цене 77777
                    if ($price['PRICE_TYPE_ID'] == 2) {
        ...
        <? global $USER;
        $userArGroup = CUser::GetUserGroup($USER->GetID());//$USER->GetUserGroupArray(); ?>
            ...
            <? if ($item["PROPERTIES"]["SALON"]["VALUE"] && !in_array(10, $userArGroup)): ?>
                <div class="product-item-button-container" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
                    <? if ($salon = inSalon($item["PROPERTIES"]["BRANDS"]["VALUE"])): ?>
                        <a class="btn btn_ico js-btn-basket 1"
                ...
                if (!$haveOffers) {
                    if ($actualItem['CAN_BUY']) {
                        ...
                        <div class="product-item-button-container product_id_<?= $item["ID"] ?>"
                            ...
                            <a class="btn btn_ico js-btn-basket 2" id="<?= $itemIds['BUY_LINK'] ?>"
                        ...
                        <div class="product-item-button-container">
                            <?
                            if ($item['IS_BUY_USER']) { ?>
                                <a class="btn btn_ico" id="<?= $itemIds['NOT_AVAILABLE_MESS'] ?>"
                                   href="javascript:void(0)" rel="nofollow">
                    ...
                    if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y') {
                        ...
                        <div class="product-item-button-container product_id_<?= $item["ID"] ?>">
                            <a class="btn btn_ico"
                               ...
                               style="display: <?= ($actualItem['CAN_BUY'] ? 'none' : 'block') ?>;">
                                <?= $arParams['MESS_NOT_AVAILABLE'] ?>
                            </a>
                            <div id="<?= $itemIds['BASKET_ACTIONS'] ?>"
                                 style="display: <?= ($actualItem['CAN_BUY'] ? '' : 'none') ?>;">
                                <a class="btn btn_ico js-btn-basket 3" id="<?= $itemIds['BUY_LINK'] ?>"
                                   href="javascript:void(0)" rel="nofollow">
                                    <?= ($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['MESS_BTN_BUY'] : $arParams['MESS_BTN_ADD_TO_BASKET']) ?>
                                </a>
                                <span class="hide btn btn_in_basket btn_ico">Товар в корзине</span>
                        ...
                        ?>
                        <div class="product-item-button-container">
                            <a class="btn btn-themes" href="<?= $item['DETAIL_PAGE_URL'] ?>">
