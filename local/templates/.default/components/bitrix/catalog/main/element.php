...
if (in_array($arResult['VARIABLES']['ELEMENT_CODE'], $arParams['HIDE_ELEMENTS_CODE'])) {//hide present
    $arResult['VARIABLES']['ELEMENT_ID'] = 0;
    $arResult['VARIABLES']['ELEMENT_CODE'] = "";
}
?>

<div class="prod-info cl">
    <? $APPLICATION->ShowViewContent('slider_element'); ?>
    ...
    $rs = CIBlockElement::GetList(
        $arSort,
        array("ACTIVE" => "Y", "CATALOG_AVAILABLE" => "Y", "IBLOCK_ID" => $arParams["IBLOCK_ID"], "CHECK_PERMISSIONS" => "Y", "SECTION_CODE" => $arResult["VARIABLES"]["SECTION_CODE"]),
        false,
        array("nElementID" => $elementId, "nPageSize" => 1),
    ...
    <? $this->SetViewTarget('slider_element'); ?>
    <div class="container">
        <div class="prod-nav cl">
            <? if (count($arPage) == 2 && $elementId == $arPage[0]["ID"]): ?>
                <a href="<?= $arPage[1]["DETAIL_PAGE_URL"] ?>" class="prod-nav__next">Следующий товар</a>
            <? elseif (count($arPage) == 3): ?>
                <a href="<?= $arPage[2]["DETAIL_PAGE_URL"] ?>" class="prod-nav__next">Следующий товар</a>
                <a href="<?= $arPage[0]["DETAIL_PAGE_URL"] ?>" class="prod-nav__prev">Предыдущий товар</a>
            <? elseif (count($arPage) == 2 && $elementId == $arPage[1]["ID"]): ?>
                <a href="<?= $arPage[0]["DETAIL_PAGE_URL"] ?>" class="prod-nav__prev">Предыдущий товар</a>
            <? endif; ?>
        </div>
    </div>
        ...
        <div class="prod-other">
            <? if (!empty($section_id)): ?>
                ...
                $fSections = CIBlockSection::GetList(
                    false,
                    Array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "ID" => $section_id),
                    false,
                    Array("UF_ID_SECTION_OLD"), false);
                $flSections = $fSections->Fetch();
                ?>
                <div <?= (!empty($flSections["UF_ID_SECTION_OLD"]) ? 'class="prod-other-wrp cl"' : '') ?>>

                    <div class="<?= (!empty($flSections["UF_ID_SECTION_OLD"]) ? "prod-other__item" : "viewed-prods-wrp") ?>">
                        <div class="h2">похожие товары</div>
                        <?
                        global $arrFilter5;
                        $arrFilter5 = array("!ID" => $elementId, "SECTION_ID" => \Kosmos\Multisite::getCatalogSections());
                        ?>
                        <? $APPLICATION->IncludeComponent(
                            "bitrix:catalog.section",
                            "like",
                            array(
                                "GMI_FLAG" => (!empty($flSections["UF_ID_SECTION_OLD"])) ? "N" : "Y",
                    ...
                    <? if (!empty($flSections["UF_ID_SECTION_OLD"])): ?>
                        <div class="prod-other__item">
                            <h2>Актуальные товары</h2>
                            <?
                            global $arrFilter4;
                            $arrFilter4 = array("!ID" => $elementId);
                            ...
                            $APPLICATION->IncludeComponent(
                                ...
                                array(
                                    ...
                                    "RCM_TYPE" => "personal",
                                    "SECTION_CODE" => "",
                                    "SECTION_ID" => $flSections["UF_ID_SECTION_OLD"],
                                    "SECTION_ID_VARIABLE" => "SECTION_ID",
                                    "SECTION_URL" => "",
                                    "SECTION_USER_FIELDS" => array(
                                        0 => "",
